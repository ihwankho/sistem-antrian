@extends('layouts.app')

{{-- Menambahkan CSS baru --}}
@push('styles')
    <link rel="stylesheet" href="{{ asset('css/loket/loket.css') }}">
@endpush

@section('content')
{{-- Tag <main> dihapus --}}
<div class="card main-card p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold">Manajemen Loket</h2>
        {{-- Style inline dihapus, class .btn-primary dari CSS baru sudah mencakup ini --}}
        <a href="{{ route('loket.create') }}" class="btn btn-primary"><i class="material-icons me-2">add</i><span>Tambah Loket</span></a>
    </div>
    <div class="table-responsive">
        <table class="table table-hover">
            <thead class="table-light">
                <tr><th>#</th><th>Nama Loket</th><th class="text-center">Aksi</th></tr>
            </thead>
            <tbody>
                @forelse ($lokets as $index => $loket)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $loket['nama_loket'] }}</td>
                    <td class="text-center">
                        <a href="{{ route('loket.edit', $loket['id']) }}" class="btn btn-warning btn-action me-1"><i class="material-icons">edit</i></a>
                        
                        {{-- Style inline diganti dengan class .form-delete-inline --}}
                        <form action="{{ route('loket.destroy', $loket['id']) }}" method="POST" class="form-delete-inline">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-action" onclick="return confirm('Anda yakin?')"><i class="material-icons">delete</i></button>
                        </form>
                    </td>
                </tr>
                @empty
                <tr><td colspan="3" class="text-center p-5">Belum ada data loket.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
</div>
@endsection